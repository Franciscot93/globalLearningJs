...
const validationSystem=(input,validationRules)=>
 {
    ...
    if(input.value=='' ){
      document.getElementById(`${input.name}Error`).classList.add('errorInput')
      document.getElementById(`${input.name}Error`).classList.remove('errorInputctive')
      document.getElementById(`${input.name}Fine`).classList.remove('itsFineInputActive')
      document.getElementById(`${input.name}Fine`).classList.add('itsFineInput')
...
formInputs.forEach(input=> input.addEventListener('keyup',(e)=>{
  validationSystem(input,validationRules)
}))
 

const users=[]
const registerUser=(nameUser, lastNameUser,emailUser,passwordUser,birthdayDateUser)=>{
...
registerForm.addEventListener('change',(e)=>{
  ...
  if(inputFields.nameUser && inputFields.lastNameUser&& inputFields.emailUser&& inputFields.passwordUser && inputFields.rePasswordUser && inputFields.birthdayUser){
